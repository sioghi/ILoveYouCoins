...
// Copyright (c) 2014 The ILoveYouCoins developers
...
#if defined(NDEBUG)
# error "ILoveYouCoins cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5397c27086852e9b6d90231a899bf4a20aec7f374130be6b364ae0ffa3d103f1");
uint256 hashGenesisBlockTestNet("0x61d256c870cb1482691e26c9610d2c324c0ff7453135414682b432806049c215");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // ILoveYouCoins: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "ILoveYouCoins Signed Message:\n";
...
unsigned int nStakeMaxAge = 24 *  60 * 60; // 24 hours = 1 day
...
{
    // ILoveYouCoins: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if (nHeight == 0) {
        // Genesis block
        nSubsidy = 1 * COIN;
    } else if (nHeight >= 1 && nHeight <= 102) {
        // Premine: 51% of ILY are for Crowd Funding, Finance of Goods and Faucets
        nSubsidy = 70000000 * COIN;
    } else if (nHeight >= 103 && nHeight <= 5000) {
        // 0 COINS TO PREVENT BIG MINERS MINE FIRST
        nSubsidy = 0 * COIN;
    } 
...
const int64 nTargetSpacing = 1 * 60; // ILoveYouCoins: 1 minute block
static const int64 nTargetTimespan = 2 * 60; // ILoveYouCoins: Not used
...
void ThreadScriptCheck() {
    RenameThread("iloveyoucoins-scriptch");
...
{
    // ILoveYouCoins: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
    ...
    BOOST_FOREACH(const PAIRTYPE(int, CBlockIndex*)& item, vSortedByHeight)
    {
       ...
       // ppcoin: calculate stake modifier checksum
        ...
        /*if (!CheckStakeModifierCheckpoints(pindex->nHeight, pindex->nStakeModifierChecksum))
            return error("LoadBlockIndexDB(): Failed stake modifier checkpoint height=%d, modifier=0x%016"PRI64x, pindex->nHeight, pindex->nStakeModifier); AFTER CREATE INDEX, REMOVE COMMENT OUT*/
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Make Love, not War";
        CTransaction txNew;
        txNew.nVersion = 1;
        txNew.nTime = 1424973624;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime = 1424973624;
        block.nBits = 0x1e0ffff0;
        block.nNonce = 753343;

        if (fTestNet)
        {
            txNew.nTime = block.nTime = 1424973628;
            block.nNonce = 397744;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x9039beba0019397b290f4ed78ecdbfaf7c167d02876e2b9791bba5987c0230a6"));
		block.print();
...
unsigned char pchMessageStart[4] = { 0xb6, 0xc3, 0xfc, 0xb2 }; // ILoveYouCoins: increase each by adding 2 to bitcoin's value.
